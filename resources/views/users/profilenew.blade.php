        ...
        ref.get().then( async function(snapshots){
            ...
            if(user.hasOwnProperty('wallet_amount') && user.wallet_amount != '' && user.wallet_amount != '0' ){
                if(currencyAtRight){
                    ...
                    wallet_amount_user=user.wallet_amount.toFixed(2)+''+currentCurrency;
                }else{
                     wallet_amount_user=currentCurrency+''+user.wallet_amount.toFixed(2);
                }
            
            }else{    
            
                if(currencyAtRight){
                    wallet_amount_user='0'+currentCurrency;
                }else{
                      wallet_amount_user=currentCurrency+'0';
                }
            }
        ...
        $(".save_user_btn").click(function(){
            ...
            if(userFirstName == ''){
              ...
              window.scrollTo(0, 0);
               ...
               /*,'isActive':active*/
               database.collection('users').doc(id).update({'firstName':userFirstName,'lastName':userLastName,'email':email,'phoneNumber':userPhone,'profilePictureURL':photo,'role':'customer','shippingAddress':shippingAddress,'active':active}).then(function(result) {
